         ...
         <form action="lost.do" method="get">
            <div class="contents">
                ...
                <div class="contents-container">
                	<c:choose>
                		<c:when test="${empty provList}">
							<p>---등록된 글이 없습니다---</p>
						</c:when>
						<c:otherwise>
							<c:forEach var="list" items="${provList}">
                	...
                	<c:choose>
                		<c:when test="${pageNum !=0}">
                			<c:forEach var="page" begin="1" end="${pageNum}">
                				<input type="button" value="${page}" onclick="location.href='<%=request.getContextPath() %>/lost.do?command=searchByProv&province=${prov}&page=${page}';">
                			</c:forEach>
                		</c:when>
               		</c:choose>
           		</div>
